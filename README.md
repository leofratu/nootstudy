...
- Card studio: select several topics or subtopics across curriculum units, combine basic, cloze, and multiple-choice formats, and distribute a total of 1–50 cards across the selection. Preview, edit, and remove drafts before saving. Invalid formats and duplicates are rejected; failed topics are reported without silently substituting unrelated offline cards.
- Card library: search and filter saved cards by subject, several topics, and format, then practise the selected set immediately. Scheduled review and session practice share selectable multiple-choice answers and concealed cloze blanks.
- Exam marker: submit a question, an answer, a maximum mark, and an optional mark scheme. Feedback includes criterion scores, answer evidence, corrections, and next steps. Scores are validated against the maximum; results without a supplied scheme are labelled practice estimates. Saved feedback uses the existing backed-up ARIA conversation history and does not become a school grade.
- Paper-and-ink workspace: warm light and dark themes, grouped sidebar navigation, a study desk with direct practice actions, subject tiles, and a two-column curriculum browser.
- Settings: consistent sections with adaptive navigation, explicit reasoning choices, provider-specific model controls, and visible save failures. Existing preferences and study data are preserved.
