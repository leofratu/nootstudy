...
struct BiologyContinuationTests {
    ...
    @Test func queueColdAndWarmReadsRetainNonQueueMetadata() throws {
        ...
        let cold = try ReviewDailyLimitPolicy.currentCards(in: reader)
        let warm = try ReviewDailyLimitPolicy.currentCards(in: reader)
        #expect(cold.map(\.id) == warm.map(\.id))
    ...
    @Test func queueIncludesPendingInsertEditAndDelete() throws {
        ...
        context.delete(a)
        #expect(Set(try ReviewDailyLimitPolicy.currentCards(in: context).map(\.id)) == [b.id])
        try context.save()
        #expect(Set(try ReviewDailyLimitPolicy.currentCards(in: context).map(\.id)) == [b.id])
    }
    ...
    @Test func registeredReadMatchesOrdinaryFetchAfterAnotherContextSaves() throws {
        ...
        try writer.save()
        let reader = ModelContext(store)
        _ = try ReviewDailyLimitPolicy.currentCards(in: reader)
        card.front = "Changed prompt"
        let added = StudyCard(topicName: "New", front: "New prompt", back: "New answer")
        writer.insert(added)
        try writer.save()
        let actual = Dictionary(try ReviewDailyLimitPolicy.currentCards(in: reader).map { ($0.id, $0.front) }, uniquingKeysWith: { first, _ in first })
        ...
        writer.delete(added)
        try writer.save()
        #expect(!ReviewDailyLimitPolicy.currentCards(in: reader).contains { $0.id == added.id })
    }
}
